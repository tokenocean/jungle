...
<div class="mt-6">
    ...
    {#if artwork.asset}
      <div class="flex">
        ...
        <a
          ...
          class="secondary-color my-auto ml-1 truncate">{artwork.asset}</a
        >
      </div>
    {/if}
  </div>
  ...
  <div class="text-sm text-gray-600 break-all mb-6">
    <div class="flex">
      <img src="/ipfs.svg" class="w-8 my-auto" />
      <a
        href={`https://ipfs.io/ipfs/${artwork.filename}`}
        class="secondary-color my-auto ml-1 truncate">{artwork.filename}</a
      >
    </div>
  </div>
